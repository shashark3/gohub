...
var Logger *zap.Logger //全局logger对象
...
func InitLogger(filename string, maxSize, maxBackup, maxAge int, compress bool, logType string, level string) {
	...
	if err := logLevel.UnmarshalText([]byte(level)); err != nil {
		fmt.Println("日志初始化有误，日志级别设置有误。请修改config/log.go中的log.level配置项")
	}
	//初始化core
	core := zapcore.NewCore(gerEncoder(), writeSyncer, logLevel)
...
// getLogWriter 日志记录介质. 在gohub中使用了两种介质，os.stdout和Log文件
